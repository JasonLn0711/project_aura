...
The evidence path is [`artifacts/asr-benchmark/2026-07-13-common-voice24-minimum/`](../artifacts/asr-benchmark/2026-07-13-common-voice24-minimum/), with the complete event trail in [`docs/audit-events/2026-07-14-gpu-only-asr-live-benchmark/audit-event.md`](audit-events/2026-07-14-gpu-only-asr-live-benchmark/audit-event.md). That packet preserves the historical comparison. Active ASR validation is availability-first: one explicitly activated real inference records the pinned runtime identity, ordered events, output, errors, and a functional decision without model accuracy or latency comparison. Preprocessing-effect research is a separate activation path with one fixed ASR runtime and reviewed ground truth.
