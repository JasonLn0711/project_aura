...
def _result(backend: str, status: str, output_path: Path | None, note: str) -> EnhancementResult:
...
def _enhance_with_deepfilternet(input_path: Path, output_path: Path) -> EnhancementResult:
    ...
    return _result(DEEPFILTERNET3_BACKEND, "ok", output_path, "deep-filter CLI")


def _enhance_with_clearvoice(input_path: Path, output_path: Path) -> EnhancementResult:
    try:
    ...
    return _result(CLEARVOICE_BACKEND, "ok", output_path, "ClearVoice MossFormer2_SE_48K")
...
def enhance_import_audio_if_available(
    ...
    if backend is None:
        return _result("none", "not_requested", None, "no model-based import enhancement for this mode")

    try:
        if backend == DEEPFILTERNET3_BACKEND:
            return _enhance_with_deepfilternet(input_path, output_path)
        if backend == CLEARVOICE_BACKEND:
            return _enhance_with_clearvoice(input_path, output_path)
    except Exception as exc:
        return _result(backend, "skipped", None, str(exc))

    return _result(backend, "skipped", None, f"unsupported backend: {backend}")
